...
module OmniAuth
  module Strategies
    class OpenIDConnectTest < StrategyTestCase
      ...
      def test_facebook_workplace_callback_phase_with_id_token
        code = SecureRandom.hex(16)
        state = SecureRandom.hex(16)
        nonce = SecureRandom.hex(16)
        id_token = File.read('test/fixtures/id_token.txt')
        request.stubs(:params).returns('code' => code, 'state' => state, 'id_token' => id_token)
        request.stubs(:path_info).returns('')

        strategy.options.issuer = 'example.com'
        strategy.options.client_options.force_token_type = "facebook_workplace_id_token"
        strategy.options.client_signing_alg = :RS256
        strategy.options.client_jwk_signing_key = File.read('test/fixtures/jwks.json')
        strategy.options.response_type = 'code'
